...
export default function SectionCompany({ title }) {
  ...
  const getCpn = async () => {
    setLoading(true);
    try {
      // Sử dụng sort=top để lấy công ty hàng đầu dựa trên số người theo dõi và số bài tuyển dụng
      const res = await makeRequest.get("/company?page=1&limit=8&sort=top");
